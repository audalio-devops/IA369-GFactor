...
- `backend/Dockerfile`: multi-stage Maven build → `eclipse-temurin:17-jre-alpine` runtime. Talks to `db` over the internal compose network (`SPRING_DATASOURCE_URL=jdbc:postgresql://db:5432/...`, set as an env var in `docker-compose.yml` — this overrides `application.yml`'s `localhost:5369` value since Spring env vars win over yml). Published on host port 8369 mainly for debugging (e.g. `curl`/Postman); the browser never talks to it directly. Deliberately does **not** run `mvn dependency:go-offline` as a separate caching layer — that goal resolves dependencies for *every* plugin goal in the pom, including the AWS SDK that `spring-boot-maven-plugin` only needs for `build-image`/buildpacks (never invoked here), which made the image build take several times longer for no benefit.
- `frontend/Dockerfile`: multi-stage `npm run build` (with `VITE_API_URL=/api` baked in as a **relative** path, not the server's IP) → served by `nginx:alpine`. `frontend/nginx.conf` reverse-proxies `location /api/` to `http://backend:8369/api/` over the internal network, so the whole app is reachable through **one public port**: host `3000` → container `80`.
- Because `VITE_API_URL` is relative, the built frontend image is portable across hosts/IPs/domains without a rebuild — only port 3000 needs to be opened in the OCI security list/firewall; 8369 and 5369 can stay internal-only.
