-- Migration V4: Redesenho da Modelagem para o Final do Sprint 1
-- Esta migração substitui as tabelas iniciais por uma estrutura mais robusta com soft delete, versionamento e auditoria.

-- 0. Limpeza de tabelas legadas que serão recriadas com nova estrutura
DROP TABLE IF EXISTS titulos CASCADE;
DROP TABLE IF EXISTS borderos CASCADE;

-- Extensão para geração de UUIDs, se necessário, embora usemos BigInt sequencial para numeração de borderôs corporativos
...
CREATE TABLE titulos (
    ...
    data_vencimento_ajustada DATE NOT NULL, -- Considera feriados e float D+
...
-- 4. AUDITORIA IMUTÁVEL (Sem update, sem delete)
...
-- Índices Estratégicos para Performance e Compliance
